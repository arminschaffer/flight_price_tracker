...
def mock_search() -> SearchSchema:
    ...
    return SearchSchema(
        ...
        origin="VIE",
        destination="LHR",
        earliest_departure=date(year=2100, month=1, day=1),
        latest_departure=date(year=2100, month=1, day=2),
        earliest_return=date(year=2100, month=1, day=15),
        latest_return=date(year=2100, month=1, day=30),
    )
...
def mock_flights() -> list[FlightSchema]:
    return [
        FlightSchema(
            ...
            departure_date=date(year=2100, month=1, day=1),
            return_date=date(year=2100, month=1, day=30),
...
def mock_connection() -> ConnectionSchema:
    return ConnectionSchema(
        ...
        destination="LHR",
        departure_date=date(year=2100, month=1, day=1),
        return_date=date(year=2100, month=1, day=5),
        stay_duration=4,
        max_stops=1,
